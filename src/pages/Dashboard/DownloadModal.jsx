...
//       <DialogTitle>Export data: {measurement}</DialogTitle>
//       <DialogContent>
...
//               {loading ? <CircularProgress size={20} /> : "Show data"}
//             </Button>
//           </Box>
...
//         {/* Results */}
//         <Box mt={2}>
//           <Typography variant="subtitle2">Results ({rows.length})</Typography>
//           <Box mt={1} style={{ maxHeight: 320,  minHeight: 120, overflow: "auto" }}>
...
// Perfectly fine code without units
...
// }) {
// //   const DEVICE_FIELDS_MAP = {
// //   OMD: ["temperature (°C)", "humidity (%)", "NH3 (ppm)", "H2S (ppm)", "odor (%)"],

// //   GLMD: ["leakage", "temperature", "humidity"],

// //   TMD: ["temperature (°C)", "humidity (%)"],

// //   AQIMD: ["AQI", "temperature (°C)", "humidity (%)"],
// // };
...
//   GLMD: {
//     leakage: { label: "Gas Leakage", unit: "boolean" }, // or %
//     temperature: { label: "Temperature (°C)", unit: "°C" },
//     humidity: { label: "Humidity (%)", unit: "%" },
//   },
...
//   DEVICE_FIELDS_CONFIG[deviceType] || {}
// );
...
//   // NEW – interval state
//   const [intervalValue, setIntervalValue] = useState("");           // integer string, e.g. "1"
//   const [intervalUnit,  setIntervalUnit]  = useState("h");          // "m" | "h" | "d"
//   const [intervalEnabled, setIntervalEnabled] = useState(false);   // whether the user wants intervals
...
//     return `${n}${intervalUnit}`;      // e.g.  "1h"  "30m"  "7d"
...
//   /**
//    * Main data query.
//    * When intervalFluxSuffix is set we inject aggregateWindow() before pivot
//    * so that each returned row is the MEAN of readings within that window.
//    */
//   const queryInflux = async (startISO, endISO) => {
//     if (!influxUrl || !influxToken || !influxOrg) {
//       throw new Error("Influx env vars are not set (VITE_INFLUX_URL/TOKEN/ORG).");
...
//     // aggregateWindow line – only injected when the user chose an interval
...
//     const formatTimeForCSV = (v) => (v ? new Date(v).toLocaleString() : "");
//     const escape           = (v) => `"${String(v).replace(/"/g, '""')}"`;

//     // Extra column header when interval mode is active
...
//               sx={{ width: 110 }}
//               InputProps={{
//                 endAdornment: intervalEnabled && intervalValue
//                   ? undefined
//                   : undefined,
//               }}
//               placeholder="e.g. 1"
