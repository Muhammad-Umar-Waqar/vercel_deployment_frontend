...
import { Pencil, Trash, Menu } from "lucide-react";
import Swal from "sweetalert2";
import { fetchAllVenues, updateVenue, deleteVenue, fetchVenuesByOrganization } from "../../slices/VenueSlice";
import { useDispatch, useSelector } from "react-redux";
import "../../styles/pages/management-pages.css";
import TableSkeleton from "../../components/skeletons/TableSkeleton";
import {
...
const VenueList = ({ onVenueSelect, selectedVenue }) => {
  ...
  const [venueName, setVenueName] = useState("");
  const [venueId, setVenueId] = useState(null);
  const [orgId, setOrgId] = useState(""); // used in edit modal

  // NEW: organization filter state (for the list)
  const [orgFilter, setOrgFilter] = useState(""); // "" means show all
  ...
  // if the orgFilter changes, fetch venues for that org (so venuesByOrg will populate)
  useEffect(() => {
    if (orgFilter) {
    ...
    <div className="ListPage venue-list-container bg-white rounded-xl shadow-sm w-full h-full border border-[#E5E7EB]">
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between">
          ...
          {/* NEW: Organization filter select (default = All) */}
          <div>
